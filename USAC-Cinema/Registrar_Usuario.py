...
class ListaUsuarios:
    ...
    def cargar_desde_xml(self, Nombre_Archivo):
        self.cabeza = None
        arbol = ET.parse(Nombre_Archivo)
        raiz = arbol.getroot()
        for usuario in raiz.iter("Usuario"):
            nombre = usuario.find("Nombre").text
            apellido = usuario.find("Apellido").text
            telefono = usuario.find("Teléfono").text
            correo = usuario.find("Correo").text
            contraseña = usuario.find("Contrasena").text
